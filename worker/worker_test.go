...
func TestWorkerProcess_ProcessNextJob(t *testing.T) {
	...
	t.Run("recovers from processor panic without crashing the loop", func(t *testing.T) {
		rawJob := json.RawMessage(`{"id": 1, "type": "test_job"}`)
		jobID := int64(123)
		jobType := "test_job"
		...
		mockWorkerImpl.On("PopNextJob", ctx).Return(rawJob, nil).Once()
		mockWorkerImpl.On("ParseJob", ctx, rawJob).Return(jobID, jobType, nil).Once()
		// Process が panic してもワーカーは停止しないこと
		mockJobProcessor.On("Process", ctx, jobType, rawJob).Run(func(args mock.Arguments) {
			panic("processor exploded")
		}).Return(nil).Once()
		// パニックしたジョブは best-effort で失敗としてマークされること
		mockWorkerImpl.On("FailJob", ctx, jobID, mock.Anything).Return(nil).Once()

		assert.NotPanics(t, func() {
			processed := w.processNextJob(ctx)
			// panic 回復時は false を返し、次の Tick まで待機する
