...
var (
	...
	// ErrTokenClaimSetFailed はトークンへのクレーム設定に失敗した場合のエラーです。
	ErrTokenClaimSetFailed = ergo.NewSentinel("failed to set token claim")
...
func (maker *TokenMaker) CreateTokenWithType(userID int64, tenantID string, roleID int64, tokenType string, duration time.Duration) (string, error) {
	...
	//
	// Set のエラーを無視すると、クレームが欠落したトークンを「正常」として発行して
	// しまう。受信側は user_id 等が取得できず検証に失敗するため、発行時点で失敗させる。
	claims := []struct {
		key   string
		value any
	}{
	...
	for _, c := range claims {
		if err := token.Set(c.key, c.value); err != nil {
			return "", ergo.Wrap(ErrTokenClaimSetFailed, c.key+": "+err.Error())
		}
	}
