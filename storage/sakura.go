...
// センチネルエラー定義
var (
	// ErrBucketNameRequired はバケット名が未指定の場合のエラー
	ErrBucketNameRequired = ergo.NewSentinel("bucket name is required")
	// ErrUnsupportedMethod は署名付きURLで未対応のHTTPメソッドが指定された場合のエラー
	ErrUnsupportedMethod = ergo.NewSentinel("unsupported method for signed url")
)
...
// DefaultSakuraStorageConfig はデフォルトのさくらオブジェクトストレージ設定を返します。
// AccessKey / SecretKey / Bucket は呼び出し側で上書きしてください。
func DefaultSakuraStorageConfig() SakuraStorageConfig {
	return SakuraStorageConfig{
