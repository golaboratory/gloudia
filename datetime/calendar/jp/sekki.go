...
//
// 各節気の日付は、日本で広く用いられている近似式
//
//	日 = int(DValue + AValue*Y) - int(Y/4)   ただし Y = (西暦年 + YearAdjustmentValue) - 1900
//
// で求めます。DValue は 1900 年における各節気の日、AValue は 1 年あたりの
// 移動量（≒ 太陽年 365.242... 日の小数部）に対応する経験的な定数です。
//
// これは天文計算による厳密な太陽黄経の算出ではなく近似計算であり、
// 1900 年から 2100 年程度の範囲を想定しています。範囲外の年や、
// 節気の切り替わり前後の日付では実際の暦と 1 日ずれる場合があります。
// 厳密な値が必要な用途では国立天文台の暦要項を参照してください。
