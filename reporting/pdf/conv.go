...
func (c *Converter) FromExcel() (string, error) {
	...
	if _, err := io.Copy(outFile, pdfStream); err != nil {
		outFile.Close()
		return "", ergo.Wrap(err, "failed to copy stream")
	}

	// 書き込み先の Close は必ずエラーを確認する。Close が失敗すると未フラッシュの
	// データが失われるため、無視すると不完全な PDF を「成功」として返してしまう。
	if err := outFile.Close(); err != nil {
		return "", ergo.Wrap(err, "failed to close output file")
	}
