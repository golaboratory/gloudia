...
func TestSendEmail_RejectsHeaderInjection(t *testing.T) {
	t.Run("CRLF in recipient is rejected before connecting", func(t *testing.T) {
		s := &SMTPSender{from: "from@example.com", host: "127.0.0.1", port: "0", timeout: time.Second}
		err := s.SendEmail("subj", "body",
			[]string{"victim@example.com\r\nBcc: attacker@evil.com"}, nil, nil, nil)
		assert.ErrorIs(t, err, ErrInvalidAddress)
	})

	t.Run("CRLF in from is rejected before connecting", func(t *testing.T) {
		s := &SMTPSender{from: "from@example.com\r\nSubject: hijacked", host: "127.0.0.1", port: "0", timeout: time.Second}
		err := s.SendEmail("subj", "body", []string{"ok@example.com"}, nil, nil, nil)
		assert.ErrorIs(t, err, ErrInvalidAddress)
	})
}
