...
// センチネルエラー定義
var (
	// ErrNoRecipientsSpecified は送信先が指定されていない場合のエラーです。
	ErrNoRecipientsSpecified = ergo.NewSentinel("no recipients specified")
	// ErrEmailBuildFailed はメールメッセージの構築に失敗した場合のエラーです。
	ErrEmailBuildFailed = ergo.NewSentinel("failed to build email")
	// ErrSSLConnectionFailed はSSL接続に失敗した場合のエラーです。
	ErrSSLConnectionFailed = ergo.NewSentinel("SSL connection failed")
	// ErrSMTPClientCreationFailed はSMTPクライアントの作成に失敗した場合のエラーです。
	ErrSMTPClientCreationFailed = ergo.NewSentinel("SMTP client creation failed")
	// ErrTCPConnectionFailed はTCP接続に失敗した場合のエラーです。
	ErrTCPConnectionFailed = ergo.NewSentinel("TCP connection failed")
	// ErrSTARTTLSFailed はSTARTTLSのアップグレードに失敗した場合のエラーです。
	ErrSTARTTLSFailed = ergo.NewSentinel("STARTTLS failed")
	// ErrSMTPAuthenticationFailed はSMTP認証に失敗した場合のエラーです。
	ErrSMTPAuthenticationFailed = ergo.NewSentinel("SMTP authentication failed")
	// ErrMailFromFailed はMAIL FROMコマンドの実行に失敗した場合のエラーです。
	ErrMailFromFailed = ergo.NewSentinel("MAIL FROM failed")
	// ErrRCPTToFailed はRCPT TOコマンドの実行に失敗した場合のエラーです。
	ErrRCPTToFailed = ergo.NewSentinel("RCPT TO failed")
	// ErrDATACommandFailed はDATAコマンドの実行に失敗した場合のエラーです。
	ErrDATACommandFailed = ergo.NewSentinel("DATA command failed")
	// ErrMessageWriteFailed はメッセージの書き込みに失敗した場合のエラーです。
	ErrMessageWriteFailed = ergo.NewSentinel("failed to write message")
	// ErrDataWriterCloseFailed はDATAライターの閉じ処理に失敗した場合のエラーです。
	ErrDataWriterCloseFailed = ergo.NewSentinel("failed to close DATA writer")
	// ErrInvalidAddress はメールアドレス/表示名に不正な文字 (CR/LF) が含まれている場合のエラーです（ヘッダーインジェクション対策）。
	ErrInvalidAddress = ergo.NewSentinel("invalid email address: contains CR/LF (header injection)")
)
...
func (s *SMTPSender) SendEmail(subject string, content string, to []string, cc []string, bcc []string, attachFiles []string) error {
	...
	if len(to) == 0 && len(cc) == 0 && len(bcc) == 0 {
		return ErrNoRecipientsSpecified
	}

	// ヘッダーインジェクション (CRLF) 対策:
	// 差出人/宛先 (From/To/Cc/Bcc) に改行が含まれていないか検証する。
	// net/smtp は MAIL/RCPT コマンド引数の CR/LF を検証するが、buildMessage が
	// 組み立てる From/To/Cc ヘッダーは検証対象外のため、ここで明示的に拒否する。
	allAddrs := []string{s.from}
	allAddrs = append(allAddrs, to...)
	allAddrs = append(allAddrs, cc...)
	allAddrs = append(allAddrs, bcc...)
	for _, addr := range allAddrs {
		if strings.ContainsAny(addr, "\r\n") {
			return ergo.Wrap(ErrInvalidAddress, fmt.Sprintf("address=%q", addr))
		}
	}
	...
	if err != nil {
		return ergo.Wrap(ErrEmailBuildFailed, err.Error())
	}
...
func (s *SMTPSender) send(recipients []string, message []byte) error {
	...
	if s.useSSL {
		...
		if err != nil {
			return ergo.Wrap(ErrSSLConnectionFailed, err.Error())
		}
		client, err = smtp.NewClient(conn, s.host)
		if err != nil {
			conn.Close() // クライアント生成失敗時に接続をリークさせない
			return ergo.Wrap(ErrSMTPClientCreationFailed, err.Error())
		}
	} else {
		// 標準的な TCP 接続と STARTTLS によるアップグレード
		conn, err := net.DialTimeout("tcp", addr, s.timeout)
		if err != nil {
			return ergo.Wrap(ErrTCPConnectionFailed, err.Error())
		}
		client, err = smtp.NewClient(conn, s.host)
		if err != nil {
			conn.Close() // クライアント生成失敗時に接続をリークさせない
		...
		if ok, _ := client.Extension("STARTTLS"); ok {
			...
			if err = client.StartTLS(tlsConfig); err != nil {
				client.Close() // STARTTLS 失敗時に接続をリークさせない
				return ergo.Wrap(ErrSTARTTLSFailed, err.Error())
			}
		}
	}
	// メッセージが受理されたかどうかは下の w.Close() の成否で確定するため、
	// QUIT の失敗は送信結果に影響しない。ただし QUIT が失敗した場合でも
	// 接続をリークさせないよう Close へフォールバックする。
	defer func() {
	...
	if s.username != "" {
		...
		if err = client.Auth(auth); err != nil {
			return ergo.Wrap(ErrSMTPAuthenticationFailed, err.Error())
		}
	}

	// トランザクション開始
	if err = client.Mail(s.from); err != nil {
		return ergo.Wrap(ErrMailFromFailed, err.Error())
	}
	for _, rec := range recipients {
		if err = client.Rcpt(rec); err != nil {
			return ergo.Wrap(ErrRCPTToFailed, err.Error())
		}
	}
	...
	w, err := client.Data()
	if err != nil {
		return ergo.Wrap(ErrDATACommandFailed, err.Error())
	}
	if _, err = w.Write(message); err != nil {
		return ergo.Wrap(ErrMessageWriteFailed, err.Error())
	}
	if err = w.Close(); err != nil {
		return ergo.Wrap(ErrDataWriterCloseFailed, err.Error())
	}
